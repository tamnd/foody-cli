...
describes what users get. Pick a format with `-o`, or let foody choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
foody <command> -o table   # aligned columns for reading
foody <command> -o jsonl   # one JSON object per line, for piping
foody <command> -o json    # a single JSON array
foody <command> -o csv     # spreadsheet friendly
foody <command> -o tsv     # tab-separated
foody <command> -o url     # just the URL column
foody <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
foody <command> --fields id,title,url
```
...
```bash
foody <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
foody <command>            # a table, because this is a terminal
foody <command> | wc -l    # JSONL, because this is a pipe
```
